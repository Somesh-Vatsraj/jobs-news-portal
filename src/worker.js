...
async function handle(request, env, ctx) {
  ...
  // Static assets (CSS/JS) fall through to worker asset binding
  ...
  {
    const m = matchRoute('/jobs/:slug', path);
    if (m && method === 'GET') return jobDetailRoute(request, env, m.slug);
  }
  {
    const m = matchRoute('/apply/:slug', path);
    if (m && method === 'GET') return applyRedirectRoute(request, env, m.slug);
  }

  // News
  if (method === 'GET' && path === '/news') return newsListRoute(request, env);
  {
    const m = matchRoute('/news/:slug', path);
    if (m && method === 'GET') return newsDetailRoute(request, env, m.slug);
  }
  ...
  {
    const m = matchRoute('/api/jobs/:slug', path);
    if (m && method === 'GET') return jobsApiGet(request, env, m.slug);
  }
  if (method === 'GET' && path === '/api/news') return newsApiList(request, env);
  {
    const m = matchRoute('/api/news/:slug', path);
    if (m && method === 'GET') return newsApiGet(request, env, m.slug);
  }
  ...
  if (path.startsWith('/admin/')) {
    // All /admin pages require session
    const session = await getSession(request, env);
    if (!session) {
    ...
    {
      const m = matchRoute('/admin/jobs/:id', path);
      if (m && method === 'GET') return adminJobEditRoute(request, env, session, m.id);
    }
    if (path === '/admin/news' && method === 'GET') return adminNewsListRoute(request, env, session);
    {
      const m = matchRoute('/admin/news/:id', path);
      if (m && method === 'GET') return adminNewsEditRoute(request, env, session, m.id);
    }
